import antlr4 from "antlr4";
import { createRequire } from "module";
import ZScriptLexer from "./ZScriptLexer.js";
import ZScriptParser from "./ZScriptParser.js";
import ZTranspiler from "./transpiler.js";

const require = createRequire(import.meta.url);

function formatError(filePath, ctx, message) {
    const token = ctx.start;
    return `${filePath}:${token.line}:${token.column}\nerror: ${message}`;
}

export async function processComptime(source, filePath, targetInfo) {
    if (!source || source.trim() === "") return source;
    
    let currentSource = source;
    let iterations = 0;
    const MAX_ITERATIONS = 100;

    // Restricted context for security
    const comptimeContext = {
        ZScript: {
            emit: (code) => { }, // Will be set per node
            path: filePath,
            target: targetInfo
        },
        console: {
            log: console.log,
            error: console.error,
            warn: console.warn,
            info: console.info
        },
        fetch,
        // Blocked globals for security (shadowing)
        process: {
            platform: process.platform,
            arch: process.arch,
            env: {} // Restricted env access
        },
        Bun: undefined,
        global: undefined,
        globalThis: undefined,
        require // Still needed for some library functionality, but carries risk
    };

    while (iterations < MAX_ITERATIONS) {
        const input = new antlr4.InputStream(currentSource);
        const lexer = new ZScriptLexer(input);
        const tokens = new antlr4.CommonTokenStream(lexer);
        const parser = new ZScriptParser(tokens);
        const tree = parser.program();

        const comptimeNodes = [];

        const findComptime = (node) => {
            if (node.constructor.name === "ComptimeStmtContext" || 
                node.constructor.name === "ComptimeVarDeclContext") {
                comptimeNodes.push(node);
            }
            if (node.children) {
                for (const child of node.children) findComptime(child);
            }
        };
        findComptime(tree);

        if (comptimeNodes.length === 0) break;

        const node = comptimeNodes[0];
        let currentEmission = "";
        comptimeContext.ZScript.emit = (code) => { currentEmission += code; };

        if (node.constructor.name === "ComptimeStmtContext") {
            const block = node.block();
            const transpiler = new ZTranspiler(tokens, currentSource);
            const blockJs = transpiler.visit(block);
            
            const AsyncFunction = Object.getPrototypeOf(async function(){}).constructor;
            const contextKeys = Object.keys(comptimeContext).join(", ");
            const fn = new AsyncFunction("context", `
                const { ${contextKeys} } = context;
                ${blockJs}
            `);
            try {
                await fn(comptimeContext);
            } catch (e) {
                throw new Error(formatError(filePath, node, `Error executing comptime block: ${e.message}`));
            }
        } else {
            const varDeclContent = node.varDeclContent();
            const kindNode = varDeclContent.LET() || varDeclContent.VAR() || varDeclContent.CONST();
            const kind = kindNode.getText();
            const pattern = varDeclContent.bindingPattern();
            let name = null;
            if (pattern.anyIdentifier && typeof pattern.anyIdentifier === 'function') {
                const res = pattern.anyIdentifier();
                if (res) name = res.getText();
            } else if (pattern.Identifier && typeof pattern.Identifier === 'function') {
                const res = pattern.Identifier();
                if (res) name = res.getText();
            }
            const expr = varDeclContent.expression();
            if (expr && name) {
                const transpiler = new ZTranspiler(tokens, currentSource);
                const exprJs = transpiler.visit(expr);
                
                const AsyncFunction = Object.getPrototypeOf(async function(){}).constructor;
                const contextKeys = Object.keys(comptimeContext).join(", ");
                const fn = new AsyncFunction("context", `
                    const { ${contextKeys} } = context;
                    return ${exprJs};
                `);
                try {
                    const val = await fn(comptimeContext);
                    comptimeContext[name] = val; 
                    currentEmission = `${kind} ${name} = ${JSON.stringify(val)};`;
                } catch (e) {
                    throw new Error(formatError(filePath, node, `Error executing comptime variable '${name}': ${e.message}`));
                }
            } else if (expr && !name) {
                // Destructuring not supported in comptime let yet
                currentEmission = ""; 
            }
        }

        const start = node.start.start;
        const stop = node.stop.stop;
        currentSource = currentSource.slice(0, start) + currentEmission + currentSource.slice(stop + 1);
        
        iterations++;
    }
    return currentSource;
}
